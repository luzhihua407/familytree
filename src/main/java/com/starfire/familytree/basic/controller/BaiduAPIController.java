...
//    private CemeteryMapper cemeteryMapper;
    ...
    public Response<JsonNode> readTextFromImage() throws IOException {
        ...
        JsonNode body=null;
            File file=new File("D:\\Myself\\familly");
            if(file.isDirectory()){
                File[] files = file.listFiles();
                for (int i = 0; i < files.length; i++) {
                    File file1 = files[i];
                    body= readFile(file1,contents);
                }
            }else{
                body=readFile(file,contents);
            }
        Response<JsonNode> response = new Response<JsonNode>();
        return response.success(body);
    }
    private JsonNode readFile(File file,List<String> contents) throws IOException {
...
//            String url="https://aip.baidubce.com/rest/2.0/ocr/v1/general";
        ...
        FileUtils.writeLines(new File("D:/Myself/1.txt"),contents);
